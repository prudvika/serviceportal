...
<script type="text/javascript"
	src="<c:url value="/dashboardsettings.js" />"></script>
<header>
...
<section id="list">
	...
	<div class="mode-toolbar">
		<div class="left">
			...
			<c:forEach items="${dashboardObjectList}" var="dashboardObject"
				varStatus="status">
				<c:set var="btnClass" value="" />
				<c:choose>
					<c:when test="${status.first}">
				...
				<c:if
					test="${dashboardObject.name==param.dashboardtype || (null==param.dashboardtype && dashboardObject.name==dashboardObjectMapping.dashboardType)}">
					<c:set var="btnClass" value="${btnClass}${' active'}" />
	...
	<details open id="dashboardMapping">
		<summary>
			Map
			<c:out value="${deviceTypeList[deviceTypeIndex].name}" />
			To
			<c:out value="${dashboardObjectList[dashboardObjectIndex].name}" />
		</summary>


		<sf:form method="POST" action="settings">
			<input type="hidden" name="type"
				value="<c:out value="${deviceTypeList[deviceTypeIndex].name}" />" />
			<input type="hidden" name="dashboardtype"
				value="<c:out value="${dashboardObjectList[dashboardObjectIndex].name}" />" />
			<c:forEach
				items="${dashboardObjectList[dashboardObjectIndex].properties}"
				var="property">
				<details>
					<summary>
						Mappings for "
					...
					<button type="button"
						onclick="addMapping('<c:out value="${property.name}" />')">Add
						Mapping</button>
					...
					<table id="<c:out value="${property.name}" />-table">
						...
						<c:forEach
							items="${dashboardObjectMapping.mappings[property.name]}"
							var="dashAttrMapping" varStatus="status">
							<c:set var="dashAttrMappingIndex" value="${status.index}" />
							<tr>
								<td><select
									name="<c:out value="${'device-'}${property.name}${'-'}${status.index}" />">
										<option value="" />
										<c:forEach
											items="${deviceTypeList[deviceTypeIndex].attributes}"
											var="attribute">
											<c:choose>
												<c:when
													test="${attribute.name==dashAttrMapping.deviceAttribute}">
													<option value="<c:out value="${attribute.name}" />"
														selected><c:out value="${attribute.name}" /></option>
												</c:when>
												<c:otherwise>
													<option value="<c:out value="${attribute.name}" />"><c:out
								...
								<td><select
									name="<c:out value="${'operator-'}${property.name}${'-'}${status.index}" />">
										<c:choose>
								...
								<td><input type="text"
									name="<c:out value="${'value-'}${property.name}${'-'}${status.index}" />"
									value="<c:out value="${dashAttrMapping.deviceValue}" />" /></td>
								<td><c:choose>
										<c:when test="${'java.awt.Color'==property.type}">
											<input type="color"
												name="<c:out value="${'constant-'}${property.name}${'-'}${status.index}" />"
						...
						<c:if test="${-1==dashAttrMappingIndex}">
							<tr>
								<td><select
									name="<c:out value="${'device-'}${property.name}${'-0'}" />">
										<option value="" />
										<c:forEach
											items="${deviceTypeList[deviceTypeIndex].attributes}"
											var="attribute">
								...
								<td><select
									name="<c:out value="${'operator-'}${property.name}${'-0'}" />">
										<option value="&gt;">&gt;</option>
										<option value="&gt;=">&gt;=</option>
										<option value="==">==</option>
										<option value="&lt;=">&lt;=</option>
